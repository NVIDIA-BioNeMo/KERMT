## Description: <br>
Convert a grover_base checkpoint (encoder-only or encoder + vocab heads) into a hybrid checkpoint by adding a randomly-initialized cMIM decoder + latent_dist, then continue pretraining on the user's corpus as hybrid (vocab + contrast). <br>
...
Apache-2.0 <br>
## Use Case: <br>
Developers and computational chemists who want to convert existing grover_base pretrained checkpoints into hybrid (vocab + cMIM contrastive) checkpoints and continue pretraining on a custom molecular corpus. <br>
...
**Other Properties Related to Output:** [Produces an upgraded hybrid checkpoint file and a run.json reproducibility manifest] <br>
...
## Evaluation Tasks: <br>
4 evaluation tasks (3 positive, 1 negative) executed in isolated k8s-sandbox pods with 3 attempts per task. <br>
